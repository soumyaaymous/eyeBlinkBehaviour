...
void write_data_line( )
{
    ...
    int imaging = digitalRead( IMAGING_TRIGGER_PIN );
...
void do_trial( unsigned int trial_num, trial_type_t_ ttype)
{
    ...
    write_data_line( );
    while( millis( ) - trial_start_time_ < TONE_START_TIME ) /* PRE_ time */
        write_data_line( );
    ...
    /* Tone for 350 milli-seconds. */
    unsigned toneDuration = TONE_END_TIME - TONE_START_TIME; 
    stamp_ = millis();
    sprintf( trial_state_, "TONE", 4);
    play_tone( toneDuration, 1.0);


    /*-----------------------------------------------------------------------------
     *  TRACE
     *-----------------------------------------------------------------------------*/
    unsigned traceDuration = PUFF_START_TIME - TONE_END_TIME;
    stamp_ = millis( );
...
void loop()
{
    ...
    for (size_t i = 1; i <= 100; i++) 
    {
        ...
        // ITI
        unsigned long iti = 10000 + random( 0, 5001);
        unsigned long stamp_ = millis( );
        sprintf( trial_state_, "ITI_" );
        while( millis( ) - stamp_ <= iti )
            write_data_line( );
